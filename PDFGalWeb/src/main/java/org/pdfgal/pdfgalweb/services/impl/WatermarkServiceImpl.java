...
public class WatermarkServiceImpl implements WatermarkService {
	...
	@Override
	public DownloadForm putWatermark(final MultipartFile file,
			final String text, final CustomColor customColor,
			final Float alpha, final WatermarkPosition watermarkPosition,
			final String pages, final HttpServletResponse response)
			throws Exception {

		DownloadForm result = new DownloadForm();

		if (!file.isEmpty() && StringUtils.isNotBlank(text)
				&& customColor != null && alpha != null
			...
			try {
				...
				this.pdfGal.putWatermark(inputUri, outputUri, text,
						customColor.getColor(), alpha, watermarkPosition,
						pagesList);
	...
	private List<Integer> getPages(final String pages) {
		...
		while (st.hasMoreElements()) {
			...
			try {
				...
				try {
					final Integer firstPage = Integer
							.parseInt(secondTokenFirst);
