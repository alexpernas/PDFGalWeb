...
public class ReindexServiceImpl implements ReindexService {
	...
	public DownloadForm reindex(final MultipartFile file, final List<Integer> pagesList,
			final List<NumberingStyle> numberingStylesList, final HttpServletResponse response)
			throws Exception {

		DownloadForm result = new DownloadForm();

		if (file != null && CollectionUtils.isNotEmpty(pagesList)
				&& CollectionUtils.isNotEmpty(numberingStylesList) && response != null) {
		...
		if (!(CollectionUtils.isEmpty(pagesList) && CollectionUtils.isEmpty(numberingStylesList))) {
			...
			if (CollectionUtils.isEmpty(pagesList) || CollectionUtils.isEmpty(numberingStylesList)
				...
				for (int i = 0; i < pagesList.size(); i++) {
					...
					final PDFGalPageNumbering pdfGalPageNumbering = new PDFGalPageNumberingVO(page,
							numberingStyle);
